...
- Multi-image chaining: to use EVERY uploaded image (Kling accepts only a first and optional last frame per task), a tour of N images is split into N-1 segments where segment i spans consecutive images (first_frame = image i, last_frame = image i+1). A single-image tour is one single-frame segment. Each image is thus the last frame of one segment and the first frame of the next, guaranteeing visual continuity. Per-segment duration is 5s (a single-image tour uses the AI-chosen 4–15s length); one job still counts as exactly one video against the plan allowance regardless of segment count. Note the provider cost scales with segments (N-1 paid Kling tasks per tour).
- Each segment uses the globally unique, deterministic `external_task_id` `estatetour-generation-{databaseJobId}-seg-{index}`. Segment submission POSTs are never automatically retried; because the id is derived purely from (job, segment), a lost response is reconciled by external ID on the next poll instead of re-POSTing. An ambiguous (network) submission keeps the job `processing`; a definitive rejection fails the whole job.
- Poll: a single batch `GET /tasks?external_task_ids=seg0,seg1,…` reconciles all segments. When ALL segments have `succeeded`, their outputs are downloaded in order and concatenated into one MP4 with ffmpeg (each segment scaled/padded to a common 1080p canvas, constant fps, H.264, no audio), then archived to private Supabase Storage (Kling clears outputs after 30 days). A single failed segment fails the job. The app continues polling rather than using `callback_url`, so no unauthenticated callback route is exposed.
- `ffmpeg` must be present at runtime for concatenation (2+ image tours). It is installed on Railway via `nixpacks.toml` (`aptPkgs = ["ffmpeg"]`). A single-segment tour needs no concatenation.
- Legacy single-task jobs (created before multi-segment, identified by a stored `providerTaskId`) keep the original single-clip poll/archive path for backward compatibility.
- All photos also influence the Inworld property analysis and final prompt. The official Image-to-Video endpoint still receives only first/last frame references per segment.
...
- Runtime binary: `ffmpeg` (installed via `nixpacks.toml`) is required to concatenate multi-image tour segments.
